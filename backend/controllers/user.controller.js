...
export const getUser = async (req, res) => {
    ...
    try {
        const user = await User.findOne( userid );
        if (!user) {
            return res.status(200).json({success: false, message: "User not found", data: {username: "Deleted user", linkedin: "", instagram: "", github: "", discord: "", email: ""}});    
        }
        res.status(200).json({success: true, message: "User fetched successfully", data: {username: user.username, linkedin: user.linkedin, 
            instagram: user.instagram, github: user.github, discord: user.discord, email: user.email}});
    } catch (error) {
        res.status(500).json({success: false, message: "Server error"});
    }
}
